...
const int N = 256;
const int M = 1000;
const int maxAmount = 100; //макс. число пользователей на сервере
...
DWORD WINAPI chat(LPVOID clientSocket)
{
	...
	while (true)
	{
		...
		if (!strcmp(szReq, "s"))
		{
			...
			szResp[0] = '\0';
			strcat(szResp, names[cur]);
			strcat(szResp, " left the chat");
			for (i = 0; i < curAmount; i++)
			{
...
int main()
{
	...
	while (true)
	{
		...
		if (!strcmp(names[curAmount], "admin_shut"))
		{
		...
		else
		{
			...
			if (curAmount < maxAmount)
			{
				...
				NewClient[0] = '\0';
				strcat(NewClient, "New client, ip: ");
				strcat(NewClient, inet_ntoa(from.sin_addr));
				strcat(NewClient, "; Name: ");
				strcat(NewClient, names[curAmount]);
				printf_s("%s\n", NewClient);
				curAmount++;
				//рассылаем всем клиентам сообщение о новом пользователе, кроме него самого
				for (i = 0; i < curAmount; i++)
				{
			...
			{
				// если достигнут максимум, отправляем пользователю сообщение о
				// достижении максимума и закрываем сокет
				printf("Maximum amount of clients\n");
				retVal = send(clientSock, "Sorry, too much people on the line", N, 0);
				closesocket(clientSock);
				printf("Connection closed\n");
			}
			//чат
